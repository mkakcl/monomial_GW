...
DEFAULT_CAPS = {"water": 23, "lithium-hydride": 23, "ozone": 31}
...
def _row(gw, order, nelectron):
    ...
    # Per sector, so a build that over-supplies one of them is not hidden behind a max.
    extra = {k: v["supplied"] - v["requested"] for k, v in by_sector.items()}
    surplus = "/".join(str(extra[k]) for k in SECTORS) if any(extra.values()) else ""
    ...
    row = {
        "order": order,
        # `10` where the recurrence was handed exactly what it was asked to conserve,
        # `10+1/0` where the build supplied more than the recurrence could use, per sector.
        "asked": next(iter(requested)),
        "asked_label": f"{next(iter(requested))}" + (f"+{surplus}" if surplus else ""),
        "by_sector": by_sector,
...
def _span(orders, step):
    ...
    return ", ".join(
        f"K={r[0]}" if len(r) == 1 else f"K={r[0]} to {r[-1]}" for r in _runs(orders, step)
    )
...
def _residual_report(rows, stride):
    ...
    if unmeasured:
        lines.append(
            f"  WARNING: no residual was calculated for {', '.join(sorted(unmeasured))}, so "
            "this sweep says nothing about whether those realizations reproduce their "
            "moments - a healthy-looking table here is an absence of evidence"
        )
    if not blown:
        return lines
    ...
    for (sector, conserved), orders in sorted(by_realization.items()):
        ...
        distinct += len(runs)
        parts.append(f"{sector} at {conserved} conserved over {_span(orders, stride)}")
...
def _classify_residuals(before, reached):
    ...
    Three outcomes, because collapsing them loses the fact that matters. A sector loosening
    ...
    -------
    caused, already : dict
        Sector to residual, for blow-ups the loosening created and ones that predate it.
    unknown : set
        Sectors whose residual is `nan` at either end, so nothing can be said.
    """
    caused, already, unknown = {}, {}, set()
    for sector, entry in reached.items():
        ...
        if math.isnan(after) or math.isnan(prior):
            ...
            already[sector] = after
        else:
            caused[sector] = after
    return caused, already, unknown
...
def _psd_probe(mf, moments, integrals, order, loosen):
...
def run(name, cap, xc=DEFAULT_XC):
    ...
    for order in range(1, cap + 1, stride):
        ...
        if row["homo_ev"] is None:
            print(
                f"{row['order']:3d} {row['asked_label']:>5s} "
    ...
    for row in (r for r in rows if r["shortfall"]):
        ...
        try:
            reached = _psd_probe(
                mf, tuple(m[: row["order"] + 1] for m in moments), integrals, row["order"], LOOSEN
            )
        except LayoutError:
        ...
        # Group on structure, never on printed residuals: to three significant figures those
        # move with the cap the moments were built at and with the BLAS summation order, so
        # keying on text would split a run of identical findings into one line each for a
        # reason that is not a finding. `verdict` embeds those digits, so it is rebuilt here
        # from the facts it is made of rather than reused.
        key = (
            _sectors(before, "conserved"),
            _sectors(reached, "conserved"),
            tuple(sorted(caused)),
    ...
    if marked:
        print(
            ...
            "still-live measurement. cons and resid also freeze there and carry no mark, "
            "because they describe the pinned realization rather than compare two of them; "
            "short keeps growing, being the gap to a request that keeps rising"
        )
...
def main():
    ...
    for name in args.systems:
        ...
        if cap is None:
            ...
            cap = measured if measured is not None else DEFAULT_CAP
            if measured is None:
