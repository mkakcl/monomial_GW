...
- [ ] Evaluate a direct Chebyshev/modified-moment plus block-Jacobi backend for orders
  ...
  **The premise no longer holds** `[2026-08-12]`. Everything above was measured against a
  ...
  sector leaves the residual band entirely from `nmom_max = 19` onward** - eighteen orders of
  magnitude above the 5-8e-15 the parking rests on, found one order past where the earlier
  sweep ended. Note what this is and is not: the correction above is about the same
  molecule's *hole* sector, where the limit turned out to be a gate policy and where the
  pencil's own frontier agrees to 5.6e-7 eV. This is the particle sector, it is a
  reconstruction failure rather than a conserved-order one, and neither finding carries over
  to the other.
  So the trigger has fired and the instruction below is met rather than standing. What it
  does not establish is whether the blow-up is the basis or the molecule - the cross-check
  against `mkakcl/chebyshev-gw` is the right instrument, run on the particle sector this
